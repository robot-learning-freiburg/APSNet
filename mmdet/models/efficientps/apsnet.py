...
class APSNet(BaseDetector):
    def __init__(
        ...
        self.mask_head_occluder = builder.build_head(mask_head_occluder)
        self.mask_head_occlusion = builder.build_head(mask_head_occlusion)
        self.mask_head_visible = builder.build_head(mask_head_visible)
        self.transform_head = builder.build_head(transform_head)
    ...
    def init_weights(self, pretrained=None):
        ...
        self.mask_head_occluder.init_weights()
        self.mask_head_occlusion.init_weights()
        self.mask_head_visible.init_weights()
    ...
    def forward_train(
        ...
        gt_masks_occluder=None,
        gt_masks_occlusion=None,
    ):
        losses = dict()
        if self.with_extra:
        ...
        if mask_feats.shape[0] > 0:
            ...
            fused_feats = torch.cat([fused_feats, mask_feats], dim=1)
            mask_pred_amodal, amodal_feats = self.mask_head_amodal(fused_feats)
            visible_feats = torch.cat([visible_feats, amodal_feats], dim=1)
            mask_pred = self.mask_head(visible_feats)
            ...
            losses["loss_mask_occluder"] = loss_mask_occluder["loss_mask"]
            losses["loss_mask_visible"] = loss_mask_visible["loss_mask"]
            losses["loss_mask_occlusion"] = loss_mask_occlusion["loss_mask"]
            loss_mask = self.mask_head.loss(mask_pred, mask_targets, pos_labels)
            losses.update(loss_mask)
        return losses

    def forward_train_occlusion(
        self,
        mask_feats,
        sampling_results,
        gt_masks,
        ...
        pos_labels = torch.cat([res.pos_gt_labels for res in sampling_results])
        occluder_pred, occluder_feats = self.mask_head_occluder(mask_feats)
        mask_targets_occluder = self.mask_head_occluder.get_target(
            sampling_results, gt_masks_occluder, self.train_cfg.rcnn
        )
        visible_pred, visible_feats = self.mask_head_visible(mask_feats)
        mask_targets_visible = self.mask_head_visible.get_target(
            sampling_results, gt_masks, self.train_cfg.rcnn
        )
        occluder_vis_feats = torch.cat(
            [occluder_feats, visible_feats, mask_feats], dim=1
        )
        occlusion_pred, occlusion_feats = self.mask_head_occlusion(occluder_vis_feats)
        mask_targets_occlusion = self.mask_head_occlusion.get_target(
            sampling_results, gt_masks_occlusion, self.train_cfg.rcnn
        )
        occ2_vis_feats = torch.cat(
            [occluder_feats, visible_feats, occlusion_feats], dim=1
        )
        fused_feats = self.transform_head(occ2_vis_feats)
        loss_mask_occluder = self.mask_head_occluder.loss(
            occluder_pred, mask_targets_occluder, pos_labels
        )
        loss_mask_visible = self.mask_head_visible.loss(
            visible_pred, mask_targets_visible, pos_labels
        )
        loss_mask_occlusion = self.mask_head_occlusion.loss(
            occlusion_pred, mask_targets_occlusion, pos_labels
        )
        return (
            fused_feats,
            visible_feats,
    ...
    def forward_test_occlusion(self, mask_feats):
        occluder_pred, occluder_feats = self.mask_head_occluder(mask_feats)
        visible_pred, visible_feats = self.mask_head_visible(mask_feats)
        occluder_vis_feats = torch.cat(
            [occluder_feats, visible_feats, mask_feats], dim=1
        ...
        visible_feats = torch.cat([visible_feats, amodal_feats], dim=1)
        mask_pred = self.mask_head(visible_feats)
        return mask_pred_amodal, mask_pred
    ...
    def simple_test_amodal_panoptic(
        ...
        if det_bboxes.shape[0] == 0:
            ...
            mask_pred_amodal, mask_pred = self.forward_test_occlusion(mask_feats)
            idx, bbx_inv, bbx_idx = self.get_bboxes_info(
                det_bboxes, _bboxes, mask_pred, ref_size
            )
