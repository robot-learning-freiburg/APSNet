...
from cityscapesscripts.evaluation.evalPanopticSemanticLabeling import (
    average_pq, pq_compute_multi_core)
from mmcv.utils import print_log

from mmdet.core.evaluation.dataset_labels import asd_labels
from amodal_panoptic_eval import evaluate as evaluate_amodal_panoptic
...
class AmodalSynthDriveDataset(CocoDataset):
    CLASSES = ("person", "car", "truck", "bus", "motor", "bike")
    ...
    def _parse_ann_info(self, img_info, ann_info):
        ...
        gt_masks_ann = []
        gt_masks_ann_amodal = []
        gt_masks_ann_occluder = []
        gt_masks_ann_occlusion = []
        ...
        for i, ann in enumerate(ann_info):
            ...
            x1, y1, w, h = ann["amodal_bbox"]
            bbox_amodal = [x1, y1, x1 + w - 1, y1 + h - 1]
            ...
            if ann.get("iscrowd", False):
                ...
                gt_masks_ann_amodal.append(ann["amodal_segmentation"])
                gt_bboxes_amodal.append(bbox_amodal)
                if "occluder_segmentation" in ann:
                    gt_masks_ann_occluder.append(ann["occluder_segmentation"])
                if "occlusion_segmentation" in ann:
                    gt_masks_ann_occlusion.append(ann["occlusion_segmentation"])
                    

        if gt_bboxes:
            gt_bboxes = np.array(gt_bboxes, dtype=np.float32)
            gt_bboxes_amodal = np.array(gt_bboxes_amodal, dtype=np.float32)
            gt_labels = np.array(gt_labels, dtype=np.int64)
        else:
            gt_bboxes = np.zeros((0, 4), dtype=np.float32)
            gt_bboxes_amodal = np.zeros((0, 4), dtype=np.float32)
        ...
        ann = dict(
            ...
            masks=gt_masks_ann,
            bboxes_amodal=gt_bboxes_amodal,
            masks_amodal=gt_masks_ann_amodal,
            masks_occluder=gt_masks_ann_occluder,
            masks_occlusion=gt_masks_ann_occlusion,
            seg_map=seg_map,
        )
    ...
    def evaluate(
        ...
        if "amodal-panoptic" in metrics:
            eval_results.update(
                self._evaluate_amodal_panoptic(results, eval_dir, logger)
            )
        elif "panoptic" in metrics:
            eval_results.update(
                self._evaluate_panoptic(results, eval_dir, outfile_prefix, logger)
            )
        else: 
            raise ValueError("Unsupported evaluation type {}".format(metric))
    ...
    def _evaluate_panoptic(self, results, eval_dir, txtfile_prefix, logger):
        ...
        if not dist.is_available() or dist.is_initialized():
            pred_folder = osp.join(eval_dir, "tmpDir/tmp")
            pred_json = osp.join(eval_dir, "tmpDir/tmp_json")
        else:
            pred_folder = osp.join(eval_dir, "tmpDir/tmp")
            pred_json = osp.join(eval_dir, "tmpDir/tmp_json")
        pred_folder = "tmpDir_3/tmp"
        pred_json = "tmpDir_3/tmp_json"
        ...
        for pred_ann in os.listdir(pred_json):
            ...
            pred_annotations.update(
                {el["image_id"]: el for el in tmp_json["annotations"]}
            )
    ...
    def _evaluate_amodal_panoptic(self, results, eval_dir, logger):
        pred_folder = os.path.join(eval_dir, "amodal_panoptic_seg")
        gt_folder = self.panoptic_gt
        result_file = "result_amodal_panoptic.json"
        apq_results , apc_results = evaluate_amodal_panoptic('amodalSynthDrive', gt_folder, pred_folder, result_file)
        shutil.rmtree(eval_dir)
        eval_results = {'apq': apq_results, 'apc': apc_results, }
        return eval_results
