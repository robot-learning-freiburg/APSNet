...
class RandomCrop(object):
    ...
    def __call__(self, results):
        ...
        for key in results.get('seg_fields', []):
            ...
            if 'depth' in key:
                results[key] = np.int16(results[key])
        ...
        if 'gt_bboxes' in results:
            ...
            if 'gt_bboxes_amodal' in results:
                results['gt_bboxes_amodal'] = results['gt_bboxes_amodal'][
                    valid_inds, :]
            ...
            # filter and crop the masks
            if 'gt_masks_amodal' in results:
                valid_gt_masks = []
                for i in np.where(valid_inds)[0]:
                    gt_mask = results['gt_masks_amodal'][i][crop_y1:crop_y2,
                                                     crop_x1:crop_x2]
                    valid_gt_masks.append(gt_mask)

                if valid_gt_masks:
                    results['gt_masks_amodal'] = np.stack(valid_gt_masks)
                else:
                    results['gt_masks_amodal'] = np.empty(
                        (0, ) + results['img_shape'], dtype=np.uint8)
                    
            if 'gt_masks_occluder' in results:
                valid_gt_masks = []
                for i in np.where(valid_inds)[0]:
                    gt_mask = results['gt_masks_occluder'][i][crop_y1:crop_y2,
                                                     crop_x1:crop_x2]
                    valid_gt_masks.append(gt_mask)

                if valid_gt_masks:
                    results['gt_masks_occluder'] = np.stack(valid_gt_masks)
                else:
                    results['gt_masks_occluder'] = np.empty(
                        (0, ) + results['img_shape'], dtype=np.uint8)

            if 'gt_masks_occlusion' in results:
                valid_gt_masks = []
                for i in np.where(valid_inds)[0]:
                    gt_mask = results['gt_masks_occlusion'][i][crop_y1:crop_y2,
                                                     crop_x1:crop_x2]
                    valid_gt_masks.append(gt_mask)

                if valid_gt_masks:
                    results['gt_masks_occlusion'] = np.stack(valid_gt_masks)
                else:
                    results['gt_masks_occlusion'] = np.empty(
                        (0, ) + results['img_shape'], dtype=np.uint8)
