...
def main():
    ...
    if args.launcher == 'none':
        ...
        with tempfile.TemporaryDirectory() as eval_dir:
            print(f"Temporary directory created at: {eval_dir}")
    else:
        distributed = True
        ...
        dir_name = f"temp_dir_{port}"
        # Create a temporary directory with the exact name
        eval_dir = os.path.join('/tmp', dir_name)
    ...
    if not distributed:
        ...
        outputs = single_gpu_test(model, data_loader, eval_dir, args.show, args.eval)
    else:
        model = MMDistributedDataParallel(
            model.cuda(),
            device_ids=[torch.cuda.current_device()],
            broadcast_buffers=False)
        outputs = multi_gpu_test(model, data_loader, eval_dir, args.tmpdir,
                                 args.gpu_collect, args.eval)

    rank, _ = get_dist_info()
    if rank == 0:
        kwargs = {} if args.options is None else args.options
        if args.eval:
            dataset.evaluate(None, eval_dir, args.eval, **kwargs)
