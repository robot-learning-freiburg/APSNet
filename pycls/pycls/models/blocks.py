...
def conv2d(w_in, w_out, k, *, stride=1, groups=1, bias=False, dilation=1):
    ...
    if dilation>1:
        assert s == 1
        d = dilation
        s, p, g, b = stride, (d*(k - 1)) // 2, groups, bias
    return nn.Conv2d(w_in, w_out, k, stride=s, padding=p, groups=g, bias=b, dilation=d)


def norm2d(w_in):
    """Helper for building a norm2d layer."""
    return nn.BatchNorm2d(num_features=w_in, eps=cfg.BN.EPS, momentum=cfg.BN.MOM)
    #return nn.SyncBatchNorm(num_features=w_in, eps=cfg.BN.EPS, momentum=cfg.BN.MOM)
